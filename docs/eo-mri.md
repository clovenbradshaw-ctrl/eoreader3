...
inject one of four scenarios. Nothing in it is scripted frame-to-frame: the
operators light because events with their signatures occur, and the order-check
reads the firings.
...
## Forthcoming: real turns

Today EO-MRI runs four **illustrative** scenario traces. The component does not
hardcode them in a way that blocks live data — it renders whatever trace map it
is handed (the `traces` prop). The seam for the real wiring is
`window.EOMRI.traceFromTurn(turn)`, which today returns `null` (and the
instrument falls back to the scenarios).

Two engine updates feed it, both already specified in
`docs/reading-conformance.md`:

- **/EO reader compliance** — per-turn conformance scoring of the Act face (the
  seven invariants; `tools/conformance.js` already scores ingestion *dumps*).
  When a turn carries its compliance verdict, the order-check renders the real
  result rather than the derived-from-trace one.
- **3-fold address encoding** — when the log carries per-event
  `operator(Site, Resolution)` addresses (the spec: *"When logs carry site and
  stance addresses, the instrument gains those columns"*), the Site and
  Resolution faces and the address line render the engine's real addresses.
